...
| E-UI | **Resource Hub v2**: filter chips (type/domain/level), 👍/👎/bookmark on every card, personalized-ordering indicators ("because you prefer docs"), learner profile settings page (role/level/language/goals editor), self-healing proposal cards ("Module 3 rebalance?") w/ accept/dismiss; **v2.5 extras**: "Save to Dojo" import confirmation flow (tag/type suggestion before commit), native-import player state badge ("imported — journey ready") on third-party videos, restructuring status card ("reformatting your video…") with live progress | Resource Hub, roadmap views |
| F-UI | **Patterns Dashboard** (`/dashboard/patterns`): time-of-day habit heatmap, strong/weak topic map fed by the interest graph, stuck-question carousel w/ retry buttons, completion/time-per-resource charts, consent toggle screen for analytics collection (explicit opt-in, plain-language list of exactly what is collected); **Restructuring UX**: struggle-moment interstitial ("This seems tough — want it restructured for you?" accept/adjust-style), progress states while reformatting, before/after format switcher (original vs personalized beats/whiteboard/simplified); **Practice-site offer chip**: non-intrusive extension bubble "You could practice X right here" w/ start/dismiss/never-on-this-site; **Bookmark flow**: one-click Save-to-Dojo toast → tag/type suggestion → dashboard confirmation link; imported-video badge linking straight into its beat journey | Workspace dashboard, extension surfaces |
